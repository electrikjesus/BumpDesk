...
class OverlayRenderer(private val shader: DefaultShader) {
    ...
    fun drawFolderUI(vPMatrix: FloatArray, pile: Pile, closeBtnTextureId: Int, nameTextureId: Int, lightPos: FloatArray) {
        ...
        if (nameTextureId != -1) {
            Matrix.setIdentityM(modelMatrix, 0)
            Matrix.translateM(modelMatrix, 0, uiX, 2.99f, uiZ - data.totalHalfDimZ + 0.3f * pile.scale)
        ...
        // Close Button
        Matrix.setIdentityM(modelMatrix, 0)
        Matrix.translateM(modelMatrix, 0, uiX + data.halfDim - 0.2f * pile.scale, 2.99f, uiZ - data.totalHalfDimZ + 0.2f * pile.scale)
    ...
    fun drawGridScrollButtons(vPMatrix: FloatArray, pile: Pile, scrollUpId: Int, scrollDownId: Int, lightPos: FloatArray) {
        ...
        if (scrollUpId != -1) {
            Matrix.setIdentityM(modelMatrix, 0)
            Matrix.translateM(modelMatrix, 0, uiX - data.halfDim + 0.4f * pile.scale, 2.99f, uiZ - data.totalHalfDimZ + 0.2f * pile.scale)
        ...
        if (scrollDownId != -1) {
            Matrix.setIdentityM(modelMatrix, 0)
            Matrix.translateM(modelMatrix, 0, uiX - data.halfDim + 1.0f * pile.scale, 2.99f, uiZ - data.totalHalfDimZ + 0.2f * pile.scale)
            Matrix.scaleM(modelMatrix, 0, 0.25f * pile.scale, 1f, 0.25f * pile.scale)
    ...
    fun getConstrainedFolderUI(pile: Pile): FolderUIData {
        ...
        val halfDimY = 0.01f // Tiny thickness for the plane
        ...
        return FolderUIData(halfDim, totalHalfDimZ, halfDimY, floatArrayOf(uiX, 2.98f, uiZ))
    }
}
